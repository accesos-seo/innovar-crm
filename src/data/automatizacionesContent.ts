...
La Edge Function lee todos los secretos (RESEND_API_KEY, META_WABA_ACCESS_TOKEN, META_PHONE_NUMBER_ID, ADMIN_WA_PHONE) desde el Vault de Supabase — ninguna credencial está en el código fuente.`,
  ...
  {
    slug: 'notificador-cotizacion',
    nombre: 'Notificador de Cotización al Cliente',
    descripcion: 'Cuando un comercial o admin marca una cotización como enviada, se genera un token público único, se registra la fecha de vencimiento y se despacha automáticamente un mensaje WhatsApp al cliente con el enlace para ver y aprobar su cotización.',
    descripcion_larga: 'El flujo arranca cuando el usuario hace clic en "Enviar al cliente" en el detalle de una cotización en estado borrador. El RPC `send_quotation_to_client` valida el rol, asigna número de cotización (si no tiene), calcula valid_until = hoy + quotation_validity_days (sistema por defecto: 30), cambia el estado a "enviada" y encola el mensaje en `notification_queue`. Un minuto después, la Edge Function `process-whatsapp-notifications` lee la cola y llama a la API de Meta con la template `quotation_sent_v1` (3 parámetros: nombre del cliente, número de cotización, enlace público). El cliente accede sin autenticarse al portal /c/<short_code> donde ve la cotización completa y puede aprobarla. Si está activo `wa_test_phone_override` en system_settings, todos los mensajes se redirigen a ese número (modo test).',
    problema_que_resuelve: 'Elimina el paso manual de copiar el link de la cotización y enviárselo al cliente por WhatsApp. Garantiza que el cliente siempre tenga un enlace válido y con fecha de vencimiento clara.',
    beneficios: [
      'Envío automático de WhatsApp con link directo a la cotización',
      'Portal público sin login: el cliente puede ver y aprobar desde cualquier dispositivo',
      'Token de un solo uso con vencimiento configurable (default 30 días)',
      'Modo test: `wa_test_phone_override` redirige todos los mensajes a un número de prueba',
      'Historial de envíos con fecha en el campo `sent_at` de la cotización',
    ],
    casos_de_uso: [
      'Enviar cotización de remodelación de cocina al cliente',
      'Re-enviar cotización expirada tras reactivarla',
      'Probar el flujo completo en desarrollo sin molestar al cliente real',
    ],
    metricas: [
      { valor: '—', etiqueta: 'Cotizaciones enviadas' },
      { valor: '—', etiqueta: 'Tiempo promedio hasta aprobación (días)' },
      { valor: '30d', etiqueta: 'Vencimiento por defecto del enlace' },
    ],
    flujo_visual: [
      { tipo: 'trigger', label: 'Clic "Enviar al cliente"', sublabel: 'Cotización en estado BORRADOR' },
      { tipo: 'proceso', label: 'RPC send_quotation_to_client', sublabel: 'Asigna número, valid_until, status → enviada' },
      { tipo: 'proceso', label: 'INSERT notification_queue', sublabel: 'Template quotation_sent_v1' },
      { tipo: 'decision', label: 'wa_test_phone_override?', sublabel: 'Sí → redirige a número de prueba' },
      { tipo: 'api', label: 'Meta WhatsApp API', sublabel: 'Nombre + número cotización + enlace público' },
      { tipo: 'output', label: 'Cliente recibe WA', sublabel: 'Abre /c/<short_code> → ve y aprueba' },
    ],
    categoria: 'notificaciones',
    status: 'activo',
    visibilidad: 'interna',
    ...
    fuente_datos: 'quotations, notification_queue, system_settings',
    canal_salida: ['whatsapp'],
    n8n_workflow_id: '—',
    supabase_proyecto: 'xdzbjptozeqcbnaqhtye',
    responsable: 'Robert Virona',
    ultima_revision: '2026-06-11T00:00:00Z',
    pasos: [
      'Usuario hace clic en "Enviar al cliente" (solo visible para cotizaciones en estado borrador)',
      'RPC valida rol (admin / comercial / super_admin)',
      'Genera public_token UUID y short_code (8 chars)',
      'Asigna quotation_number si aún no tiene',
      'Calcula valid_until = NOW() + quotation_validity_days (de system_settings)',
      'UPDATE quotations SET status=enviada, sent_at=NOW(), public_token, valid_until',
      'INSERT notification_queue con payload {client_name, quotation_number, link}',
      'EF process-whatsapp-notifications corre cada 60s, lee pending, llama Meta API',
      'Si wa_test_phone_override está activo → todos los WA van a ese número',
      'Cliente abre el enlace /c/<short_code> en portal público (sin login)',
    ],
    notas: 'Activado 2026-06-11. Requiere VITE_FF_PHASE_4_QUOTATION_PUBLIC=true en .env para mostrar el botón. wa_test_phone_override en system_settings redirige mensajes en pruebas (actualmente apuntando a 3183061286). El enlace público en WA apunta a public_app_base_url (Vercel); en local, navegar manualmente a localhost:3000/c/<short_code>. Template Meta quotation_sent_v1 debe estar aprobada.',
    historial: [
      { fecha: '2026-06-11T00:00:00Z', descripcion: 'Flujo de envío de cotización activado. Feature flag añadida al .env. Bug de React hooks en QuotationDetail.tsx corregido (useAuthStore declarado antes de los early returns). wa_test_phone_override configurado a 3183061286.', autor: 'Robert Virona' },
    ],
    rutas_codigo: [
      'src/components/quotations/SendQuotationButton.tsx',
      'src/hooks/quotations/useSendQuotation.ts',
      'src/lib/features.ts',
      'src/pages/QuotationDetail.tsx',
      'src/pages/PublicQuotation.tsx',
      'supabase/functions/process-whatsapp-notifications/index.ts',
      'db/migrations/042_fix_welcome_lead_and_quotation_valid_until.sql',
    ],
  },
