...
import { Ruler, Box, Sparkles, MapPin, Globe, Instagram, Phone, Layers } from 'lucide-react';
import { cn } from '@/lib/utils';

interface ClosetTemplateProps {
  data: {
    client_name: string;
    total_amount: number;
    configuration: any;
...
export const ClosetTemplate: React.FC<ClosetTemplateProps> = ({ data }) => {
  ...
  return (
    <div className="w-[800px] min-h-[1100px] bg-white p-12 text-slate-900 font-sans selection:bg-primary/30 shadow-none border border-slate-100">
      ...
      <header className="flex justify-between items-start border-b-2 border-slate-900 pb-12">
        <div className="space-y-6">
          ...
          <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Propuesta De Ingeniería: Closets a Medida</p>
            <p className="text-sm font-bold font-mono">CODE: CLO-{Math.random().toString(36).substring(7).toUpperCase()}</p>
          </div>
        </div>
      ...
      <section className="py-12 grid grid-cols-2 gap-12">
        <div className="space-y-4">
          ...
          <p className="text-xs leading-relaxed text-slate-600 font-medium italic">
            "Este diseño contempla una solución de almacenamiento integral, maximizando cada centímetro cúbico 
            bajo parámetros de elegancia y ergonomía. Profundidad estándar de 0.60m para optimización de colgado."
          </p>
        </div>
        <div className="bg-slate-50 p-6 border-l-4 border-slate-900 flex flex-col justify-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-tight">Parametrización Validada m²</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-12">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-900 italic mb-10">Especificaciones Técnicas del Closet</h3>
        ...
        <div className="grid grid-cols-5 border-t border-slate-200">
           ...
           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Categoría Aplicada</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-mono font-bold text-slate-900 uppercase">
             {config.type || 'Estandar'}
           </div>
           ...
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-mono font-bold text-primary">
             {config.area?.toFixed(2)} m²
           </div>

           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Sistema de Accesos</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-bold uppercase text-slate-900 italic">
             Puertas {config.doorType === 'batiente' ? 'BATIENTES' : 'CORREDIZAS'}
           </div>

           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight text-primary">Hardware Incluido</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-[10px] font-medium text-slate-500 leading-relaxed uppercase tracking-wider">
             Maletero, divisor, doble colgadero, entrepaños, doble cajonero, zapatero y puertas.
           </div>

           {config.observations && (
              <>
                <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Notas de Diseño</div>
                <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-medium text-slate-600 italic">
                  {config.observations}
                </div>
              </>
           )}
        ...
        <div className="flex justify-between items-end relative z-10 w-full">
          <div className="space-y-4">
            ...
            <div className="flex gap-6 mt-4 opacity-70">
              <div className="space-y-1">
                <p className="text-[8px] font-bold uppercase">Base Prod.</p>
                <p className="text-xs font-mono font-bold text-white">$ {config.productSubtotal?.toLocaleString('es-CO')}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[8px] font-bold uppercase">Logística</p>
                <p className="text-xs font-mono font-bold text-white">$ {config.transport?.toLocaleString('es-CO')}</p>
              </div>
              {config.discountPercent > 0 && (
                <div className="space-y-1">
                  <p className="text-[8px] font-bold uppercase text-emerald-400">Dto. {config.discountPercent}%</p>
                  <p className="text-xs font-mono font-bold text-emerald-400">- $ {config.discountAmount?.toLocaleString('es-CO')}</p>
                </div>
              )}
            </div>
          </div>
