...
import { Tv, Sparkles, MapPin, Globe, Instagram, Phone } from 'lucide-react';

interface TVCenterTemplateProps {
  data: {
    client_name: string;
    total_amount: number;
    configuration: any;
...
export const TVCenterTemplate: React.FC<TVCenterTemplateProps> = ({ data }) => {
  ...
  return (
    <div className="w-[800px] min-h-[1100px] bg-white p-12 text-slate-900 font-sans selection:bg-primary/30 shadow-none border border-slate-100">
      ...
      <header className="flex justify-between items-start border-b-2 border-slate-900 pb-12">
        <div className="space-y-6">
          ...
          <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Propuesta De Ingeniería: Centro de TV Multimedia</p>
            <p className="text-sm font-bold font-mono">CODE: TVC-{Math.random().toString(36).substring(7).toUpperCase()}</p>
          </div>
        </div>
      ...
      <section className="py-12 grid grid-cols-2 gap-12">
        <div className="space-y-4">
          ...
          <p className="text-xs leading-relaxed text-slate-600 font-medium italic">
            "Este diseño contempla un centro de entretenimiento integrado, diseñado bajo parámetros de gestión de cableado 
            invisible y optimización de espacios multimedia. Ergonomía visual garantizada."
          </p>
        </div>
        <div className="bg-slate-50 p-6 border-l-4 border-slate-900 flex flex-col justify-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-tight">Parametrización Validada de Sistema</span>
          </div>
        </div>
      </section>
      ...
      <section className="py-12">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-900 italic mb-10">Especificaciones Técnicas del Sistema</h3>
        ...
        <div className="grid grid-cols-5 border-t border-slate-200">
           ...
           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Mueble Base</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-mono font-bold text-slate-900 uppercase">
             {config.includeBase ? 'INCLUIDO' : 'SIN MUEBLE BASE'}
           </div>

           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Acabado de Superficie</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-mono font-bold text-slate-900 uppercase">
             {config.highGloss ? 'ALTO BRILLO' : 'ESTÁNDAR (MATE/RH)'}
           </div>

           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Iluminación LED</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-mono font-bold text-primary">
             {config.ledMetros > 0 ? `${config.ledMetros} Metros Lineales` : 'NO INCLUYE'}
           </div>

           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight">Repisas Adicionales</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-xs font-mono font-bold text-slate-900">
             {config.shelvesQuantity > 0 ? `${config.shelvesQuantity} Unidades` : 'SOLO MUEBLE'}
           </div>

           <div className="col-span-2 py-6 border-b border-slate-100 text-xs font-bold uppercase tracking-tight text-primary">Material / Hardware</div>
           <div className="col-span-3 py-6 border-b border-slate-100 text-[10px] font-medium text-slate-500 leading-relaxed uppercase tracking-wider">
             Fabricado en RH 15mm o 18mm, incluye sistema de pasacables interno y herrajes de fijación a muro de alta resistencia.
           </div>
        </div>
      </section>
        ...
        <div className="flex justify-between items-end relative z-10 w-full">
          <div className="space-y-4">
            ...
            <div className="flex gap-6 mt-4 opacity-70">
              <div className="space-y-1">
                <p className="text-[8px] font-bold uppercase">Subtotal Config.</p>
