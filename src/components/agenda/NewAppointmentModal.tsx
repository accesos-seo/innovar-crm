...
export function NewAppointmentModal({
  ...
  return (
    ...
    >
      <form id="new-appointment-form" onSubmit={handleSubmit} className="space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-muted/5 p-8 border border-border/10">
                ...
                <div className="space-y-3">
                  ...
                  <Select value={staffId} onValueChange={setStaffId}>
                    <SelectTrigger className="bg-background border-border/50 h-14 rounded-none focus:ring-primary font-bold">
                      <SelectValue placeholder={isLoadingStaff ? formatSentenceCase("Cargando...") : formatSentenceCase("Seleccionar comercial...")}>
                          {staffId 
                            ? staffList.find(opt => opt.id === staffId)?.full_name || '👤 Usuario sin nombre'
                            : undefined}
                      </SelectValue>
                    </SelectTrigger>
                    <SelectContent className="bg-card border-border/50">
                      {staffList.map((staff) => (
                        <SelectItem key={staff.id} value={staff.id}>
                          {staff.full_name || '👤 Usuario sin nombre'}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
          ...
          <div className="space-y-8">
            ...
            <div className="p-6 border border-border/10">
              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-relaxed">
                El agendamiento notificará automáticamente al comercial y al cliente vía correo electrónico.
              </p>
            </div>
          </div>
        </div>
