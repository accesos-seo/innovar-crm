...
const HabilidadesSidebar: React.FC = () => (
  <nav className="py-5 px-3 flex flex-col gap-0.5 text-sm select-none">
    ...
    <p className="px-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/50 mb-1">Skills</p>
    {habilidadesData.map((item) => (
      <div key={item.id} className="flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] text-muted-foreground hover:bg-accent/50 cursor-default">
    ...
    <div className="p-6 lg:p-10 max-w-3xl mx-auto">
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${HABILIDADES_COLOR}20` }}>
            <Wand2 size={18} style={{ color: HABILIDADES_COLOR }} />
          </div>
          <div>
            <h1 className="text-2xl font-extrabold text-foreground font-heading">Habilidades</h1>
            <p className="text-[13px] text-muted-foreground mt-0.5">Skills del sistema de IA</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3">
        {habilidadesData.map((item, i) => (
          <div key={item.id} className="bg-card rounded-xl border border-border/20 p-5 hover:border-border/50 hover:shadow-[0_4px_20px_rgba(0,0,0,0.3)] transition-all">
            <div className="flex items-start gap-3 min-w-0">
              <span className="text-[11px] font-black text-muted-foreground/30 tabular-nums shrink-0 mt-0.5">
                {String(i + 1).padStart(2, '0')}
              </span>
              <div className="min-w-0 flex-1">
                <h2 className="text-[15px] font-bold leading-snug font-heading" style={{ color: BRAND_COLOR }}>
                  {item.title}
                </h2>
                ...
                {item.features && item.features.length > 0 && (
                  <ul className="mt-2.5 flex flex-wrap gap-1.5">
                    {item.features.map((f, fi) => (
                      <li key={fi} className="text-[11px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                        {f}
                      </li>
                    ))}
                  </ul>
            ...
            <div className="mt-3 pt-3 border-t border-border/10 flex items-center gap-2 text-[11px] text-muted-foreground/50">
              <Clock size={11} />
