...
| `030_phase4_quotations_new_columns.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S1** · ALTER `quotations` agregando 7 columnas (client_acceptance_note, client_rejection_reason/subtype, client_approved_at, client_rejected_at, viewed_at, view_count) + `clients.first_project_at` + CHECK constraint en subtype. |
| `031_phase4_bank_settings_seeds.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S1** · 7 filas en `system_settings` para datos bancarios (bank_name, bank_account_*, bank_holder_*, nequi/daviplata phones) editables sin deploy + helper `get_bank_setting()`. **EDITAR VALORES ANTES DE SALIR A PROD** (Alvaro lo hará desde la UI de BankSettings en Slice 3). |
| `032_phase4_designer_qa_seed.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S1** · INSERT profile diseñador QA (`diseno.test@innovar.local`, role=`diseno`, WhatsApp `+573183061286` confirmado por Alvaro) + helper `get_active_designers()`. `auth.users` row creado vía service_role API antes de aplicar. |
| `033_phase4_storage_buckets.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S1** · Buckets `payment-receipts` (5MB, image/pdf, INSERT anónimo validado por estado de quotation) + `quotation-pdfs` (10MB, solo PDF, INSERT solo service_role). RLS estricto por rol. |
| `034_phase4_public_quotation_rpcs.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S2** · 4 RPCs públicas: `get_public_quotation` (con redirect a versión vigente + tracking view_count), `accept_public_quotation`, `reject_public_quotation` (subtype adjustments_requested/declined), `request_quotation_reactivation`. **Fix in-place**: la versión inicial pasaba `template_name=NULL` a `enqueue_notification` para in-app — corregido a INSERT directo en `public.notifications`. |
| `035_phase4_lock_and_sync_triggers.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S2** · `send_quotation_to_client` RPC (draft→sent + WA al cliente, template diferenciado V2) + triggers: `lock_quotation_on_sent`, `sync_opportunity_from_quotation`, `notify_quotation_acceptance` (WA bank info al cliente), `notify_quotation_rejection`, `log_quotation_status_change` (audit) + `unlock_quotation` RPC (admin, exige change_reason). **Fix in-place**: in-app notifs ahora INSERTan directo en `notifications`, no via `enqueue_notification`. |
| `035a_phase4_quotation_short_code.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S2** · Column `quotations.short_code` UNIQUE 6-char base62 + trigger BEFORE INSERT + backfill + RPC `resolve_quotation_short_code` + re-CREATE de `get_public_quotation` (devuelve short_code) y `send_quotation_to_client` (WA al cliente usa `/c/<short_code>`). Habilita ruta `/c/:code` en el frontend. |
| `035b_phase4_admin_wa_on_quotation_actions.sql` | ✅ Applied 2026-05-23 | **Fase 4 · S2.5** · Cierre del gap WA al admin: re-CREATE de `fn_notify_quotation_acceptance` y `fn_notify_quotation_rejection` agregando `PERFORM fn_wa_enqueue_for_profile(...)` para los 3 escenarios. **Bloqueador externo**: 3 templates Meta nuevos pendientes (`admin_quotation_accepted_v1`, `admin_quotation_adjustments_v1`, `admin_quotation_rejected_v1`) — mientras tanto los rows quedan `pending`/`failed` en `notification_queue`. Smoke E2E (3 escenarios → 3 rows en queue con template correcto) verificado en prod. |
